...
class RuCCSDcomplex(UnitaryCCSD):
    ...
    def general_ccsd_generator(self,param_expression=True):
        ...
        for i in range(no):
            for a in range(nv):
                ...
                orb_iA = 2*i
                orb_aA = 2*(a+no)
                t1A = FermionOperator(((orb_aA,1),(orb_iA,0)),t1re)
                t1A += FermionOperator(((orb_aA,1),(orb_iA,0)),t1im)*1j
                
                orb_iB = 2*i+1
                orb_aB = 2*(a+no)+1
                t1B = FermionOperator(((orb_aB,1),(orb_iB,0)),t1re)
                t1B += FermionOperator(((orb_aB,1),(orb_iB,0)),t1im)*1j
