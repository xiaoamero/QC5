    ...
    def general_ccsd_generator(self,param_expression=True):
        ...
        for pA in range(nmo):
            for qA in range(pA):
                ...
                t1a = packed_amps[idx]
                usa += FermionOperator(((orb_pA,1),(orb_qA,0)),t1a)
                idx += 1
        # t1b
        usb = FermionOperator()
        for pB in range(nmo):
            for qB in range(pB):
                ...
                t1b = packed_amps[idx]
                usb += FermionOperator(((orb_pB,1),(orb_qB,0)),t1b)
                idx += 1     
        # t2aa
            ...
            for jA in range(iA):
                for aA in range(nvA):
                    for bA in range(aA):
                        ...
                        t2aa = packed_amps[idx]
                        usaa += FermionOperator(((orb_aA,1),(orb_bA,1),(orb_jA,0),(orb_iA,0)),t2aa)
                        idx += 1 
        #t2ab
            ...
            for jB in range(noB):
                for aA in range(nvA):
                    for bB in range(nvB):
                        ...
                        t2ab = packed_amps[idx]
                        usab += FermionOperator(((orb_aA,1),(orb_bB,1),(orb_jB,0),(orb_iA,0)),t2ab)
                        idx += 1     
        # t2bb
            ...
            for jB in range(iB):
                for aB in range(nvB):
                    for bB in range(aB):
                        ...
                        t2bb = packed_amps[idx]
                        usbb += FermionOperator(((orb_aB,1),(orb_bB,1),(orb_jB,0),(orb_iB,0)),t2bb)
                        idx += 1          
