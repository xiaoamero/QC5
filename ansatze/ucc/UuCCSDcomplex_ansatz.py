...
class UuCCSDcomplex(UnitaryCCSD):
    ...
    def general_ccsd_generator(self,param_expression=True):
        ...
        for iA in range(noA):
            for aA in range(nvA):
                orb_iA = 2*iA
                orb_aA = 2*(aA+noA)   
                t1A = FermionOperator(((orb_aA,1),(orb_iA,0)),packed_amps[idx])
                t1A += FermionOperator(((orb_aA,1),(orb_iA,0)),packed_amps[idx+med])*1j
                usa += t1A
                idx += 1
        # t1b
        usb = FermionOperator()
        for iB in range(noB):
            for aB in range(nvB):
                ...
                t1B = FermionOperator(((orb_aB,1),(orb_iB,0)),packed_amps[idx])
                t1B += FermionOperator(((orb_aB,1),(orb_iB,0)),packed_amps[idx+med])*1j
                usb += t1B
                idx += 1      
        # t2aa
