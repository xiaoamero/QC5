...
class UuCCGSD2complex(UnitaryCCSD):
                ...
                for p in range(r+1):
                    for q in range(p):
                        ...
                        if rs > pq: 
                            ...
                            orb_rA = 2*r
                            orb_sA = 2*s
                            t2AA = FermionOperator(((orb_rA,1),(orb_sA,1),(orb_qA,0),(orb_pA,0)),packed_amps[idx])
                            t2AA += FermionOperator(((orb_rA,1),(orb_sA,1),(orb_qA,0),(orb_pA,0)),packed_amps[idx+med])*1j
                            usaa += t2AA
                            idx += 1 
        #t2ab
                ...
                for p in range(nmo):
                    for q in range(nmo):
                        ...
                        if r_s > p_q:
                            ...
                            orb_rA = 2*r
                            orb_sB = 2*s+1
                            t2AB = FermionOperator(((orb_rA,1),(orb_sB,1),(orb_qB,0),(orb_pA,0)),packed_amps[idx])
                            t2AB += FermionOperator(((orb_rA,1),(orb_sB,1),(orb_qB,0),(orb_pA,0)),packed_amps[idx+med])*1j
                            usab += t2AB
                            idx += 1
        # t2bb
                ...
                for p in range(r+1):
                    for q in range(p):
                        ...
                        if rs > pq: 
                            ...
                            orb_rB = 2*r+1
                            orb_sB = 2*s+1
                            t2BB = FermionOperator(((orb_rB,1),(orb_sB,1),(orb_qB,0),(orb_pB,0)),packed_amps[idx])
                            t2BB += FermionOperator(((orb_rB,1),(orb_sB,1),(orb_qB,0),(orb_pB,0)),packed_amps[idx+med])*1j
                            usbb += t2BB
                            idx += 1
